...
public class BitwiseHashTransformer implements HashTransformer {
    ...
    public Expr hash(final BasicBlock vertex, PredicateFlowGetter caller) {
        ...
        // (starting & (7 << 29))
        final Expr shiftedvartoshift = new FakeArithmeticExpr(caller.get(vertex), shifted7to29, ArithmeticExpr.Operator.AND);
        ...
        final Expr shiftedStartTo3 = new FakeArithmeticExpr(caller.get(vertex), const3, ArithmeticExpr.Operator.SHL);
