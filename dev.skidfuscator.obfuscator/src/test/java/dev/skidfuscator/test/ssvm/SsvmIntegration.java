...
import dev.skidfuscator.test.util.SsvmUtil;
...
public class SsvmIntegration {
	private static final Logger logger = LogManager.getLogger(SsvmIntegration.class);
	private static final ExecutorService vmThreadPool = Executors.newSingleThreadExecutor();
	private Skidfuscator workspace;
	private LocalVmFactory factory;
	 ...
	 */
	public SsvmIntegration(Skidfuscator workspace) {
		...
		if (initialized) {
			...
			if (cause instanceof VMException) {
				...
				if (vm != null) {
					...
					logger.error(String.format("SSVM failed to initialize: %s", oop));
	 ...
	 */
	public Skidfuscator getWorkspace() {
		return workspace;
	}
	 ...
	 */
	public LocalVmFactory getFactory() {
		return factory;
	}
	 ...
	 */
	public boolean setFactory(LocalVmFactory factory) {
