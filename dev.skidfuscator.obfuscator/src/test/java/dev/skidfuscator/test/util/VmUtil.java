...
public final class VmUtil {
	...
	public void addUrl(String urlPath) {
		...
		ObjectValue ucp = vm.getPublicOperations().getReference(classLoader, klass, "ucp", urlClassPath.getDescriptor());
	...
	public ObjectValue getSystemClassLoader() {
		VirtualMachine vm = this.vm;
		JavaMethod getSystemClassLoader = vm.getPublicLinkResolver().resolveStaticMethod(vm.getSymbols().java_lang_ClassLoader(), "getSystemClassLoader", "()Ljava/lang/ClassLoader;");
	...
	public ExecutionContext invokeExact(InstanceJavaClass klass, String name, String desc, Value... args) {
		...
		JavaMethod target = vm.getPublicLinkResolver().resolveSpecialMethod(klass, name, desc);
	...
	public ExecutionContext invokeVirtual(String name, String desc, Value... args) {
		...
		JavaMethod target = vm.getPublicLinkResolver().resolveVirtualMethod(value, name, desc);
		Locals locals = vm.getThreadStorage().newLocals(target);
		locals.copyFrom(args, 0, 0, args.length);
		return helper.invoke(target, locals);
	}
	...
	public ExecutionContext invokeStatic(InstanceJavaClass klass, String name, String desc, Value... args) {
		VirtualMachine vm = this.vm;
		JavaMethod target = vm.getPublicLinkResolver().resolveStaticMethod(klass, name, desc);
		Locals locals = vm.getThreadStorage().newLocals(target);
		locals.copyFrom(args, 0, 0, args.length);
	...
	public InstanceValue newInstance(InstanceJavaClass klass, String desc, Value... args) {
		klass.initialize();
		VirtualMachine vm = this.vm;
		JavaMethod init = vm.getPublicLinkResolver().resolveSpecialMethod(klass, "<init>", desc);
		InstanceValue value = vm.getPublicOperations().allocateInstance(klass);
