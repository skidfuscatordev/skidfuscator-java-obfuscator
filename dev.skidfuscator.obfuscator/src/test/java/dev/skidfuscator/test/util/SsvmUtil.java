...
public class SsvmUtil {
	private final static Logger logger = LogManager.getLogger(SsvmUtil.class);
	...
	public static void shutdown(VirtualMachine vm, int code) {
		...
		try {
			...
			logger.error(String.format("Could not shutdown virtual machine\n%s", util.throwableToString(ex.getOop())));
		}
	}
}